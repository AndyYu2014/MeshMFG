(function () {
  ...
  async function apiCall(methodName, params, requestMethod) {
    ...
    if (method === 'GET') {
      ...
      if (csrf) options.headers['X-Frappe-CSRF-Token'] = csrf;
  ...
  async function initWorkOrderNewPage() {
    ...
    if (existingName) {
      ...
      try {
        var defaults = await apiCall('get_form_defaults', {}, 'GET');
        if (defaults.source_warehouse) {
          document.getElementById('source-warehouse').value = defaults.source_warehouse;
        }
        if (defaults.fg_warehouse) {
          document.getElementById('fg-warehouse').value = defaults.fg_warehouse;
        }
        if (defaults.wip_warehouse) {
          document.getElementById('wip-warehouse').value = defaults.wip_warehouse;
        }
      } catch (e) {
        console.error('默认仓库加载失败', e);
      }
    }
